...
<Callout type="tip">
  Every struct field maps directly to a storage key in Soroban's persistent
  storage. Each plan and subscription is its own entry, which works well with
  Soroban's parallelism model.
</Callout>
...
| Wallet          | Stellar Wallets Kit (Freighter, Lobstr) |
| Source of truth | On-chain Soroban state (no database)    |
...
The contract is live on Stellar testnet today, with mainnet deployment coming after pilot merchants shake out any remaining edge cases. The SDK works. The dashboard works. Next steps:

1. Onboard the first wave of pilot merchants on testnet
2. Stellar Community Fund submission with a working demo
3. Mainnet deployment after real-world usage validates the billing loop
4. Expand the SDK with webhook support and multi-token billing

Here is a quick look at how Soroban smart contracts work on Stellar:

<Video
  src="https://www.youtube.com/embed/iIMgaUyiEho"
  title="The Power of Stellar"
/>
